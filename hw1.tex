...
\date{Due: 1630h on Friday, 12 February}
...
We implement our custom image type as \texttt{MyImage} as shown in Listing \ref{lst:img} on Page \pageref{lst:img} and also included in the accompanying file \texttt{myimage.py}. Note that the implementation uses \texttt{MyList} which is our implementation of the list. The given implementation is complete and you are not to change it.

\subsection{List}

We implement our custom list as \texttt{MyList} whose interface shown in Listing \ref{lst:lst} on Page \pageref{lst:lst} and also included in the accompanying file \texttt{mylist.py}. Note that the implementation is mostly complete except for the segments marked as \texttt{pass}. The implementation includes a mechanism to allow iteration over the list and using some other native python syntax with it. Go over the \texttt{MyList} implementation thoroughly in order to understand what follows.

In the same file, you have to create two new classes -  \texttt{ArrayList} and \textsf{PointerList} - by copying the implementation of \texttt{MyList}. The \texttt{ArrayList} class will implement the list using \href{https://www.programiz.com/python-programming/array}{python arrays} and the \textsf{PointerList} class will implement it based on nodes. See the implementation of BST in Listing \ref{lst:bst} on Page \pageref{lst:bst} and also included in the accompanying file \texttt{bst.py} to understand nodes in python. 

Finally, you have to modify the required line in \texttt{myimage}.py to use your type.

\section{Testing}
We will be using the continuous integration tool, \texttt{Travis CI}, to test your implementation of image operations. A tutorial will be conducted by your TA's on the matter. Stay tuned for  more details.
